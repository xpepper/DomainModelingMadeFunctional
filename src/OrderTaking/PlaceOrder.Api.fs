...
let internal checkProductExists: Implementation.CheckProductCodeExists =
    fun productCode -> true // dummy implementation
...
let internal getProductPrice: Implementation.GetProductPrice =
    fun productCode -> Price.unsafeCreate 1M // dummy implementation
...
let workflowResultToHttpReponse result =
    ...
    | Ok events ->
        ...
        let dtos = events |> List.map PlaceOrderEventDto.fromDomain |> List.toArray // arrays are json friendly
...
let placeOrderApi: PlaceOrderApi =
    fun request ->
        ...
        let pricedOrderToPricedOrderWithShippingInformation =
            Implementation.pricedOrderToPricedOrderWithShippingInformationFactory
                Implementation.calculateShippingInformation
        ...
        asyncResult |> Async.map (workflowResultToHttpReponse)
